import {
  ...
  export const addDocumentFirebase = async (collectionName, data,companyId) => {
	  ...
	  console.log(data,'data di add document firebase')
  ...
  export const uploadFileFirebase = async (
	...
	const storageRef = ref(storage, `user/${currentCompanies}/${location}/${data.name}`);
  ...
  export const uploadBase64Firebase = (location,currentCompanies,name,base64)=>{
	const storageRef = ref(storage, `projects/${currentCompanies}/${location}/${name}`);
	const result = uploadString(storageRef, base64, 'data_url')
	.then(() => {
